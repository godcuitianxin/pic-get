...
def downLoad(each_pic):
    n=1
    try:
        if os.path.exists('e:\\{}_picture'.format(name)):
            os.mkdir('e:\\{}_picture_new'.format(name))
            while n<=number:
                pic = r.get(each_pic[n-1],header)
                time.sleep(0.5)
                with open('e:\\{}_picture_new\\{}_picture{}.jpg'.format(name,name,n),'wb') as f:
                    f.write(pic.content)            
